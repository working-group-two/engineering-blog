...
Timer is a corner stone of any software that communicates over the network. There are plenty of implementations
that provide timer facility. Most of them are in memory and will lose scheduled timers should the application crash.
...
Before taking on timers lets cover some Kafka grounds.

# Kafka concepts
There are few Kafka concepts in play to solve the timers - Kafka topic, Kafka stream, Kafka streams library.
...
A kafka topic resides inside the Kafka servers, but Kafka streams library keeps track of topic's most recent
...
the streams. When event appears on any of the joined streams the library retrieves events with same key from
 other streams cache maps and presents the tuple of the found events to the application.
...
is the clock dial counting seconds since UNIX epoch in the real time. At some point it will get to the second
which is equal to the key on *timer-request* topic, and the join will come up with the callback event.

Excellent summary on "timer wheel" and other timer algorithms can be found in this linux kernel [thread](https://lwn.net/Articles/156329/).
...
![](/Users/sz/COVO/omnicate.github.io/img/blog/kafka-timers/kt.png)
...
When requesting a timer by pushing *timer-request* topic user provides a callback structure 
...
Check out openings on the [Careers page](https://wgtwo.com/career) if you’re interested in building future Telco backbone.
